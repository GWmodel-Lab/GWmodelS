#include <QWidget>
#include "gwmsymboleditordialog.h"
#include <QLabel>
...
GwmSymbolEditorDialog::GwmSymbolEditorDialog(QgsVectorLayer* vectorLayer,QWidget *parent) : QDialog(parent)
{
    if(vectorLayer){
        mLayer = vectorLayer;
    }
...
//    symbolTypeSelect->setCurrentIndex( -1 );
    ...
    rendererChanged(1);
    connect( symbolTypeSelect, static_cast<void ( QComboBox::* )( int )>( &QComboBox::currentIndexChanged ), this, &GwmSymbolEditorDialog::rendererChanged );
    createButtons();
    createLayout();
}

GwmSymbolEditorDialog::~GwmSymbolEditorDialog()
{
    if (mSelectedRenderer)
        delete mSelectedRenderer;
}



void GwmSymbolEditorDialog::createLayout()
{
...
//combobox值改变 切换窗口槽函数
void GwmSymbolEditorDialog::rendererChanged(int index)
{
...
//        connect( mActiveWidget, &QgsRendererWidget::layerVariablesChanged, this, &GwmSymbolEditorDialog::layerVariablesChanged );
      }
...
void GwmSymbolEditorDialog::createButtons()
{
    ...
    connect(applyBtn,&QPushButton::clicked,this,&GwmSymbolEditorDialog::applyLayerSymbol);
    connect(cancelBtn,&QPushButton::clicked,this,&GwmSymbolEditorDialog::onCloseButtonClicked);
    connect(okBtn,&QPushButton::clicked,this,&GwmSymbolEditorDialog::onOkBtnClicked);
    btnContainer->setLayout(btnLayout);
}

void GwmSymbolEditorDialog::applyLayerSymbol()
{
    ...
    if ( renderer )
    {
       ...
       // renderer->setOrderByEnabled( checkboxEnableOrderBy->isChecked() );
        mSelectedRenderer = renderer->clone();
//        mLayer->setRenderer( renderer->clone() );
//        emit canvasRefreshSingal();
    }
}

void GwmSymbolEditorDialog::onOkBtnClicked()
{
    applyLayerSymbol();
    QDialog::accept();
}

void GwmSymbolEditorDialog::onCloseButtonClicked()
{
    QDialog::reject();
}

QgsFeatureRenderer *GwmSymbolEditorDialog::selectedRenderer() const
{
    return mSelectedRenderer;
}

void GwmSymbolEditorDialog::setSelectedRenderer(QgsFeatureRenderer *selectedRenderer)
{
