...
vec GwmGWaverageTaskThread::del(vec x, int rowcount){
    vec res;
    if(rowcount == 0)
...
bool GwmGWaverageTaskThread::isValid()
{
...
bool GwmGWaverageTaskThread::CalculateSerial(){
    mat rankX = mX;
...
#ifdef ENABLE_OpenMP
bool GwmGWaverageTaskThread::CalculateOmp(){
    mat rankX = mX;
    rankX.each_col([&](vec x) { x = rank(x); });
    int nVar = mX.n_cols, nRp = mDataPoints.n_rows;
    emit tick(0,nRp);
...
#endif
vec GwmGWaverageTaskThread::findq(const mat &x, const vec &w)
{
    int lw = w.n_rows;
    int lp = 3;
...
void GwmGWaverageTaskThread::initPoints()
{
...
void GwmGWaverageTaskThread::initXY(mat &x, const QList<GwmVariable> &indepVars)
{
...
void GwmGWaverageTaskThread::createResultLayer(CreateResultLayerData data)
{
    ...
    if(treeChildCount > 0)
    {
        layerName += QStringLiteral("_GWAverages") + "(" + QString::number(treeChildCount) + ")";
    } else
    {
        layerName += QStringLiteral("_GWAverages");
    }
    //节点记录标签
    treeChildCount++ ;
...
void GwmGWaverageTaskThread::setParallelType(const IParallelalbe::ParallelType &type)
{
...
//            mRegressionFunction = &GwmBasicGWRAlgorithm::regressionSerial;
            mCalFunciton = &GwmGWaverageTaskThread::CalculateSerial;
            break;
#ifdef ENABLE_OpenMP
        case IParallelalbe::ParallelType::OpenMP:
            mCalFunciton = &GwmGWaverageTaskThread::CalculateOmp;
            break;
#endif
        default:
            mCalFunciton = &GwmGWaverageTaskThread::CalculateSerial;
