#include "gwmdelimitedtextfile.h"
#include "qgslogger.h"
...
GwmDelimitedTextFile::GwmDelimitedTextFile( const QString &url )
  : mFileName( QString() )
...
GwmDelimitedTextFile::~GwmDelimitedTextFile()
{
  close();
}

void GwmDelimitedTextFile::close()
{
  if ( mStream )
  {
    delete mStream;
...
bool GwmDelimitedTextFile::open()
{
  if ( ! mFile )
  {
    close();
    ...
    if ( mFile )
    {
      ...
      if ( mUseWatcher )
      {
        ...
        connect( mWatcher, &QFileSystemWatcher::fileChanged, this, &GwmDelimitedTextFile::updateFile );
      }
    }
  }
...
void GwmDelimitedTextFile::updateFile()
{
  close();
  emit fileUpdated();
}
...
void GwmDelimitedTextFile::resetDefinition()
{
  close();
  mFieldNames.clear();
  mMaxFieldCount = 0;
...
bool GwmDelimitedTextFile::setFromUrl( const QString &url )
{
...
// Extract the provider definition from the url
bool GwmDelimitedTextFile::setFromUrl( const QUrl &url )
{
...
void GwmDelimitedTextFile::setFileName( const QString &filename )
{
  resetDefinition();
  mFileName = filename;
}

void GwmDelimitedTextFile::setEncoding( const QString &encoding )
{
  resetDefinition();
  mEncoding = encoding;
}

void GwmDelimitedTextFile::setUseWatcher( bool useWatcher )
{
  resetDefinition();
  mUseWatcher = useWatcher;
}

QString GwmDelimitedTextFile::type()
{
...
void GwmDelimitedTextFile::setTypeWhitespace()
{
...
void GwmDelimitedTextFile::setTypeRegexp( const QString &regexp )
{
  resetDefinition();
  mType = DelimTypeRegexp;
  mDelimRegexp.setPattern( regexp );
  mAnchoredRegexp = regexp.startsWith( '^' );
  mParser = &GwmDelimitedTextFile::parseRegexp;
...
QString GwmDelimitedTextFile::decodeChars( QString chars )
{
  chars = chars.replace( QLatin1String( "\\t" ), QLatin1String( "\t" ) );
  return chars;
}

QString GwmDelimitedTextFile::encodeChars( QString chars )
{
  chars = chars.replace( '\t', QLatin1String( "\\t" ) );
  return chars;
}

void GwmDelimitedTextFile::setTypeCSV( const QString &delim, const QString &quote, const QString &escape )
{
  resetDefinition();
  mType = DelimTypeCSV;
  ...
  mParser = &GwmDelimitedTextFile::parseQuoted;
...
void GwmDelimitedTextFile::setSkipLines( int skiplines )
{
  resetDefinition();
  mSkipLines = skiplines;
}

void GwmDelimitedTextFile::setUseHeader( bool useheader )
{
  resetDefinition();
  mUseHeader = useheader;
}

void GwmDelimitedTextFile::setTrimFields( bool trimFields )
{
  resetDefinition();
  mTrimFields = trimFields;
}

void GwmDelimitedTextFile::setMaxFields( int maxFields )
{
  resetDefinition();
  mMaxFields = maxFields;
}

void GwmDelimitedTextFile::setDiscardEmptyFields( bool discardEmptyFields )
{
  resetDefinition();
...
void GwmDelimitedTextFile::setFieldNames( const QStringList &names )
{
  mFieldNames.clear();
...
QStringList &GwmDelimitedTextFile::fieldNames()
{
...
int GwmDelimitedTextFile::fieldIndex( const QString &name )
{
  // If not yet opened then reset file to read column headers
  //
  if ( mUseHeader && ! mFile ) reset();
...
bool GwmDelimitedTextFile::setNextRecordId( long nextRecordId )
{
  if ( ! mFile ) reset();
...
GwmDelimitedTextFile::Status GwmDelimitedTextFile::nextRecord( QStringList &record )
{
...
GwmDelimitedTextFile::Status  GwmDelimitedTextFile::reset()
{
...
GwmDelimitedTextFile::Status GwmDelimitedTextFile::nextLine( QString &buffer, bool skipBlank )
{
  if ( ! mStream )
  {
    Status status = reset();
...
bool GwmDelimitedTextFile::setNextLineNumber( long nextLineNumber )
{
...
void GwmDelimitedTextFile::appendField( QStringList &record, QString field, bool quoted )
{
...
GwmDelimitedTextFile::Status GwmDelimitedTextFile::parseRegexp( QString &buffer, QStringList &fields )
{
...
GwmDelimitedTextFile::Status GwmDelimitedTextFile::parseQuoted( QString &buffer, QStringList &fields )
{
  Status status = RecordOk;
...
bool GwmDelimitedTextFile::isValid()
{
