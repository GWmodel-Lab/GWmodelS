...
GwmGWAverageTaskThread::GwmGWAverageTaskThread(const GwmAlgorithmMetaVariable& meta) : mMeta(meta)
{
    ...
    mAlgorithm.setIsAutoselectBandwidth(meta.weightBandwidthAutoselect);
...
{
    return mAlgorithm.spatialWeight().weight<gwm::BandwidthWeight>();
}
...
void GwmGWAverageTaskThread::run()
{
    ...
    try
    {
        ...
        {
            const auto& bw = mAlgorithm.spatialWeight().weight<gwm::BandwidthWeight>();
