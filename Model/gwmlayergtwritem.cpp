    ...
    : GwmLayerVectorItem(parent, vector)
{
    if (taskThread)
    {
        ...
        mWeight = GwmBandwidthWeight(*static_cast<GwmBandwidthWeight*>(taskThread->stWeight().weight()));
...
int GwmLayerGTWRItem::childNumber()
{
    if (mParentItem)
...
bool GwmLayerGTWRItem::readXml(QDomNode &node)
{
...
bool GwmLayerGTWRItem::writeXml(QDomNode &node, QDomDocument &doc)
{
...
int GwmLayerGTWRItem::dataPointsSize() const
{
    return mDataPointsSize;
}

GwmDiagnostic GwmLayerGTWRItem::diagnostic() const
{
    return mDiagnostic;
}

arma::mat GwmLayerGTWRItem::betas() const
{
    return mBetas;
}

bool GwmLayerGTWRItem::bandwidthOptimized() const
{
    return isBandwidthOptimized;
}

bool GwmLayerGTWRItem::hatmatrix() const
{
    return hasHatmatrix;
}

bool GwmLayerGTWRItem::regressionPointGiven() const
{
    return isRegressionPointGiven;
}

GwmVariable GwmLayerGTWRItem::depVar() const
{
    return mDepVar;
}

QList<GwmVariable> GwmLayerGTWRItem::indepVars() const
{
    return mIndepVars;
}
...
GwmBandwidthWeight GwmLayerGTWRItem::weight() const
{
    return mWeight;
}
